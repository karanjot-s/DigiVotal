...
contract Voting {
    ...
    function createPoll(
        ...
        for (uint i = 0; i < options.length; i++) {
            VoteOption storage option = newPoll.options[
                newPoll.option_count + 1
            ];
    ...
    function vote(uint poll_id, uint option_id) public payable {
        // Check that poll exists
        require(poll_id < polls_created, "Poll does not exist");
        // Check that option exists
        require(
            option_id < polls[poll_id].option_count,
            "Option does not exist"
        );
        // Check that user has not already voted
        require(
            polls[poll_id].votes[msg.sender] == 0,
            "User has already voted"
        );
        // Check that user has not voted in the future
        require(
            polls[poll_id].start_time <= block.timestamp,
            "User has tried voting in the future"
        );
        // Check that user has not voted in the past
        require(
            polls[poll_id].end_time >= block.timestamp,
            "User has tried voting for the past"
        );
        // Check that user has enough balance
        require(
            msg.value >= polls[poll_id].fee,
